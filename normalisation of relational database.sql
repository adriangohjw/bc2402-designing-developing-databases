...
AS (SELECT location, iso_code, continent
    FROM covid19data
    GROUP BY location);
...
AS (SELECT location, population_density, median_age, aged_65_older, aged_70_older, gdp_per_capita, 
    ...
    FROM covid19data
    WHERE (population_density, median_age, aged_65_older, aged_70_older, gdp_per_capita, 
	extreme_poverty, cardiovasc_death_rate, diabetes_prevalence, female_smokers, male_smokers, 
    handwashing_facilities, hospital_beds_per_thousand, life_expectancy, human_development_index) <> ('','','','','','','','','','','','','','')
    GROUP BY location);
...
AS (SELECT location, date, total_cases, new_cases, new_cases_smoothed, reproduction_rate
	FROM covid19data
    WHERE (total_cases, new_cases, new_cases_smoothed, reproduction_rate) <> ('','','','')
...
AS (SELECT location, date, total_deaths_per_million, new_deaths_per_million, excess_mortality
	FROM covid19data
    WHERE (total_deaths_per_million, new_deaths_per_million, excess_mortality) <> ('','','')
    GROUP BY location, date);
...
AS (SELECT location, date, icu_patients, icu_patients_per_million, hosp_patients, 
	...
	FROM covid19data
	WHERE (icu_patients, icu_patients_per_million, hosp_patients, 
	hosp_patients_per_million, weekly_icu_admissions, weekly_icu_admissions_per_million, weekly_hosp_admissions, 
	weekly_hosp_admissions_per_million) <> ('','','','','','','','')
	GROUP BY location, date);
...
AS (SELECT location, date, new_tests, total_tests, total_tests_per_thousand, new_tests_per_thousand, new_tests_smoothed, 
    ...
    FROM covid19data
    WHERE (new_tests, total_tests, total_tests_per_thousand, new_tests_per_thousand, new_tests_smoothed, 
	new_tests_smoothed_per_thousand, positive_rate, tests_per_case, tests_units) <> ('','','','','','','','','')
    GROUP BY location, date);
