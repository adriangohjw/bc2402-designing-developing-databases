...
AS (SELECT location, iso_code, continent, population_density, median_age, aged_65_older, 
...
AS (SELECT location, date, total_cases, new_cases, new_cases_smoothed, reproduction_rate
	FROM covid19data
    WHERE new_cases <> '' AND total_cases <> '' OR new_cases_smoothed <> ''
    GROUP BY location, date);

-- CREATE covid_deaths TABLE
CREATE TABLE covid_deaths
AS (SELECT location, date, total_deaths_per_million, new_deaths_per_million, excess_mortality
	FROM covid19data
    WHERE total_deaths_per_million <> '' OR new_deaths_per_million <> '' OR excess_mortality <> ''
    GROUP BY location, date);

-- CREATE covid_hospitalisation TABLE
CREATE TABLE covid_hospitalisation
AS (SELECT location, date, icu_patients, icu_patients_per_million, hosp_patients, 
	hosp_patients_per_million, weekly_icu_admissions, weekly_icu_admissions_per_million, weekly_hosp_admissions, 
	weekly_hosp_admissions_per_million
	FROM covid19data
	WHERE hosp_patients <> '' OR icu_patients <> '' OR weekly_hosp_admissions <> '' OR weekly_icu_admissions <> ''
	GROUP BY location, date);

-- CREATE covid_tests TABLE
CREATE TABLE covid_tests
AS (SELECT location, date, new_tests, total_tests, total_tests_per_thousand, new_tests_per_thousand, new_tests_smoothed, 
	new_tests_smoothed_per_thousand, positive_rate, tests_per_case, tests_units
    FROM covid19data
    WHERE new_tests <> '' OR new_tests_smoothed <> '' OR total_tests <> ''
    GROUP BY location, date);
...
AS (SELECT location, date, total_vaccinations, people_vaccinated, people_fully_vaccinated, new_vaccinations, 
	...
	FROM covid19data
	WHERE new_vaccinations <> '' AND total_vaccinations <> '' AND people_vaccinated <> '' AND new_vaccinations_smoothed <> ''
	GROUP BY location, date);
